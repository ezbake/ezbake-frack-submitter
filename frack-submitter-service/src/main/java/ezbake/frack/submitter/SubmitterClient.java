...
public class SubmitterClient {
    ...
    private void run(CmdLineParser parser) throws TException, IOException, CmdLineException, EzConfigurationLoaderException {
        ...
        try {
            if (submit) {
                if (Strings.isNullOrEmpty(pipelineId)) throw new CmdLineException(parser, "Pipeline ID required for submission");
                ...
                if (Strings.isNullOrEmpty(pipelineId)) throw new CmdLineException(parser, "Pipeline ID required for shutdown");
